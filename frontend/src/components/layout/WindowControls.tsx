...
export function WindowControls({ className = "" }) {
    const { usesCustomWindowControls } = usePlatform();

    if (!usesCustomWindowControls) {
        return null;
    }

    return <CustomWindowControls className={className} />;
}

function CustomWindowControls({ className = "" }) {
    ...
    return (
        <div className={cn("wails-no-drag flex h-full items-center", className)}>
            <button
                type="button"
                tabIndex={-1}
